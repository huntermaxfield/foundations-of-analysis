...
-\textbf{FTC Note} FTC can only be applied when function\\ is riemann integrable on interval.\\
-\textbf{$f^2$ Integrability} If {f} is integrable on $[a,b]$, then so is $f^2$.\\
-\textbf{Ex: MVT and Uni Cont} $f(x) = ln(x)$ on $[\frac{1}{2}, \infty]$\\Let $\epsilon > 0$.  Need $\delta$ s.t. $|x-y| < \delta \Rightarrow |ln(x)-ln(y)| < \epsilon$\\Note $\exists{c}$ s.t. $\frac{ln(x)-ln(y)}{x-y}=f'(c)$.  Also, \\ $\frac{1}{2} \leq c \leq \infty \Rightarrow 0 \leq \frac{1}{c} \leq 2$.  Thus, $\frac{ln(x)-ln(y)}{x-y}=\frac{1}{c} \leq 2$. \\ Now, $|ln(x)-ln(y)| \leq 2|x-y| \leq 2\delta = \epsilon$ if we set $\delta = \frac{\epsilon}{2}$.
\end{tabular} 
